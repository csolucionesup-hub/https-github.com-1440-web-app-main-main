  ...
  return (
    <div className={`glass rounded-2xl p-6 transition-all duration-300 relative group overflow-hidden ${isCompleted ? 'opacity-70' : ''}`} style={{ borderTop: `4px solid ${goal.color}` }}>
      ...
      <div className="flex justify-between items-start mb-4">
        <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${statusColors[goal.status as keyof typeof statusColors] || statusColors.paused}`}>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {/* Dropdown Menu */}
          {isMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 glass rounded-xl shadow-2xl overflow-hidden z-20 animate-fade-in">
      ...
      <h3 className={`text-xl font-bold font-heading mb-2 ${isCompleted ? 'line-through-dim' : 'text-white'}`}>
        {goal.title}
      </h3>

      {goal.description && (
        <p className={`text-slate-400 text-sm mb-6 line-clamp-2 ${isCompleted ? 'line-through-dim' : ''}`}>
          {goal.description}
        </p>
      )}
