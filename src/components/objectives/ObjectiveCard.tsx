...
export const ObjectiveCard = ({ objective, onEdit }: Props) => {
    ...
    return (
        <div
            onClick={() => onEdit?.()}
            className={`glass rounded-2xl p-6 transition-all duration-300 relative border border-white/5 cursor-pointer hover:bg-slate-800/50 hover:border-indigo-500/30 ${isCompleted ? 'opacity-60' : ''}`}
        >
            <div className="flex justify-between items-start mb-4">
                <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md border ${statusColors[objective.status] || statusColors.pending}`}>
            ...
            <div className="mb-4">
                ...
                <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                        className={`h-full rounded-full transition-all duration-500 ${isCompleted ? 'bg-emerald-500' : 'bg-gradient-to-r from-indigo-500 to-purple-500'}`}
            ...
            <ConfirmDeleteModal
                ...
                onConfirm={() => deleteObjective(objective.id)}
